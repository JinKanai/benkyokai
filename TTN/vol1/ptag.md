...
webアプリとは
==
- webブラウザ（フロントエンド）で動くのでスマホでもPCでもOK。
- ブラウザから入力
- サーバ（バックエンド）で、計算、集計、データベースとのやりとりなどを処理
- サーバでHTMLを作成してブラウザに送信
- ブラウザで表示
> ブラウザで処理してサーバはデータベースのみ、というのもある。
---
...
```
↑webサーバの負荷試験の時に書いた。あと、hタグも知ってたわ・・・。いずれにしろ、見出しと段落しか作れないひどいHTMLスキルに違いはない。
...
==
- HTMLとCSSだけでwebアプリが作れるだと・・？
- 上の記事ではgithubをデータベース代わりにしてtodoアプリを作ってました。
...
==
- HTML/CSSはチュートリアルからコピペｗ
	- bootstrapとかいう、ページの見た目を良くするやつを使ってなんとなくオシャレにできそう
	- ほとんどHTMLの知識なしで実装は進むが・・・
### データベースがわからない（今まで一切ふれたことがなかった）
- 参加者テーブルと勉強会テーブルをページに表示するとこまではすぐできた。
- どの勉強会に誰が参加したのかを紐付るのって、どうするの・・・
- djangoのORマッパーでは、レコードをpythonオブジェクトとして扱う（ちょっと何言ってるかわかんないです）

---
データベースに怖気づく日々、そして・・・
==
- リレーションとやらを使えば出来るのはなんとなくわかった。
	- 参加者ID（社員番号）を勉強会の参加者カラムに紐付ける。
- でもdjangoで実装する方法がわかりません(´・ω・｀)いやなんとなくわかるんだけど、動いてるやつが動かなくなったらやだなあ・・・
- しかし止まっていてもしょうがないので、できそうなことを実験してみよう。
...
==
- ５月終わり頃、ようやく第1号が完成！
- 見た目はクソだが（HTMLわからないので）、要件は満たしていた。 
...
- デザインはそのまんま使用
	- ここまで来てほとんどHTML/CSS触ってません（＝知識増加量ゼロ）
- 機能追加はめちゃくちゃ楽
	- 例えば表示する項目を増やしたいときは、辞書に項目を追加するだけ。
	- ただし、継承とかオーバーライドとかオブジェクト指向を分かってないと辛い。
---
...
まとめ
==
- HTML/CSSを知らなくてもwebアプリは作れる（ただし見た目はクソになる）
